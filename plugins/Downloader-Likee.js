...
let handler = async (m, { conn, usedPrefix, command, text }) => {
  if (!text) return conn.reply(
    m.chat, 
    `🍂 Ingresa la URL de Likee que deseas descargar.\n\n*Ejemplo:*\n> *${usedPrefix + command}* https://l.likee.video/v/E4JSK7`, 
    m
  );
  ...
  try {
    ...
    if (response.data) {
      ...
      let txt = `\`\`\`
乂  L I K E E  -  D O W N L O A D
\`\`\`
✩  *Creador* : ${videoData.creator}
✩  *Caption* : ${videoData.caption}

> ⚙️ Enlace con marca de agua: ${videoData.links.watermark}
> 🌸 Enlace sin marca de agua: ${videoData.links['no watermark']}`;

      await conn.sendMessage(
        m.chat, 
        { video: { url: videoData.links['no watermark'] }, caption: txt }, 
        { quoted: m }
      );
      await m.react('✅');
    } else {
